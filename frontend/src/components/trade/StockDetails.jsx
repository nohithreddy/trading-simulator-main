...
export default function StockDetails({ stock, handleFormatQuote }) {
  ...
  const buildFallbackQuote = () => {
    const price = parseFloat(stock.currentPrice || 100);
    return {
      open: price,
      high: price * 1.01,
      low: price * 0.99,
      ...
      try {
        ...
        if (quoteResponse.ok) {
          ...
          if (!quote || Object.keys(quote).length === 0) {
            ...
            handleFormatQuote(fallbackQuote);
            return;
          }
  ...
  return (
    <Grid container sx={{ mt: "4rem", display: "flex", alignItems: "stretch" }}>
      ...
      <Grid item xs={7} sx={{ display: "flex", flexDirection: "column" }}>
        <StockChart symbol={stock.symbol} rapidApiKey={rapidApiKey.current} currentPrice={state.quote.price} />
      </Grid>
    </Grid>
  );
}
