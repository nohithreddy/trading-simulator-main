...
export default function StockChart(props) {
  ...
  const buildFallbackHistory = () => {
    const basePrice = parseFloat(props.currentPrice || 100);
    const today = new Date();
    const history = [];
    for (let i = 364; i >= 0; i -= 1) {
      const date = new Date(today);
      date.setDate(today.getDate() - i);
      const drift = Math.sin(i / 18) * 0.035 + Math.cos(i / 43) * 0.02;
      const price = +(basePrice * (1 + drift)).toFixed(2);
      history.push({
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (formattedData.history && formattedData.history.length > 0) {
            ...
            setChartData(extractDataForChart(fallbackHistory, StockTimeRanges[timeRange], timeRange));
          }
        } else {
          const fallbackHistory = buildFallbackHistory();
          setPriceHistory(fallbackHistory);
